...
#Evalue un othellier, pour un joueur, en se basant sur qui a capturé les coins
def evaluate_corner(M_othellier,nom_j): 
    ...
    for i in [0,2]:
        for j in [0,2]:  
            ...
            col = int(j * 7/2)
            # est-il capturé 
            if M_othellier[ligne,col] == nom_j : 
                # si oui par moi : 60
                score += 60
            #si il n'est pas capturé    
            elif M_othellier[ligne,col] == 'vide' : 
                # est ce que je suis dans les trois autour : -20 pour chaque
                if M_othellier[ligne + 1 - i,col] == nom_j : 
                    score -= 20
                if M_othellier[ligne,col + 1 - j] == nom_j : 
                    score -= 20
                if M_othellier[ligne + 1 - i,col + 1 - j] == nom_j : 
                    score -= 20
    return score
